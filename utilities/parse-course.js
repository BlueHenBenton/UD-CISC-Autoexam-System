...
/** Consumes the path to an question (activity), produces course information for that question (activity).
 * Note: this is an async/await function. For info on how that works, see here: https://javascript.info/async-await
 */
module.exports = async function parseTags(pathToQuestion) {
  // Read the file into a string and parse it.
  const xml = await parseXmlStringAsync(await fsReadAsync(pathToQuestion + '/course/course.xml'));
